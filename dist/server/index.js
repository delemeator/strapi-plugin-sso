...
async function azureAdSignInCallback(ctx) {
  ...
  try {
    ...
    if (dbUser) {
      activateUser = dbUser;
      refreshToken = await sessionManager.generateRefreshToken(activateUser.id, void 0, { type: "refresh" });
    } else {
...
const oidcSignIn = async (ctx) => {
  ...
  ctx.session.codeVerifier = codeVerifier;
  if (!state) {
    state = crypto.getRandomValues(buffer.Buffer.alloc(32)).toString("base64url");
  }
  ctx.session.oidcState = state;
  const params = new URLSearchParams();
